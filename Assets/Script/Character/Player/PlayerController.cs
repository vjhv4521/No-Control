...
public class PlayerController : MonoBehaviour
{
    ...
    private void OnMeleeAttack(InputAction.CallbackContext context)
    {
        ...
        Debug.Log("[PlayerController] 触发攻击");

        var detector = GetComponent<PlayerAttackDetector>();
        if (detector == null) detector = GetComponentInChildren<PlayerAttackDetector>(); // 尝试子物体

        if (detector != null)
        {
            Debug.Log("[PlayerController] 调用 PlayerAttackDetector.Attack()");
            detector.Attack();
        }
        else
        {
            Debug.LogWarning("[PlayerController] 未找到 PlayerAttackDetector，使用降级检测");
            ...
            foreach (Collider2D enemy in hitEnemies)
            {
                ...
                var cb = enemy.GetComponentInParent<Game.Character.CharacterBase>();
                if (cb != null)
                {
                    Debug.Log($"[PlayerController] 降级检测命中: {cb.name}, 造成10点伤害");
                    cb.TakeDamage(10f);
                }
            }
        }
